...
import { AssetSubCategory, AssetSubSubCategory, asset } from '../../../models';

export interface AssetService {
  createAsset(row: IAsset, userId: number): Promise<{ success: boolean; inserted_rows: number }>;
  // getAssets: (
...
class AssetsService implements AssetService {
...
async createAsset(
  rows: IAsset,
  userId: number
): Promise<{ success: boolean; inserted_rows: number }> {

  if (!Array.isArray(rows) || rows.length === 0) {
    throw new BadRequestException('Asset rows missing');
  }
const asset_id = uuidv4();
const insertRows = rows.map(r => ({
  assetId: asset_id,
  assetCategoryId: r.asset_category_id,
  assetSubcategoryId: r.asset_subcategory_id,
  ...
  updated_by: userId,
   ...
   const result = await this.assetModel.bulkCreate(insertRows);
